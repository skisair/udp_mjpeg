...
def disp(host:str, port:int, show_graph):
    global df
    # ipv4を使うので、AF_INET
...
def main():
    ...
    host = st.sidebar.text_input('host:', value=HOST_NAME)
    port = st.sidebar.number_input('port：', value=PORT)
    show_graph = st.sidebar.checkbox('show graph', value=False)
    disp(host, port, show_graph)
